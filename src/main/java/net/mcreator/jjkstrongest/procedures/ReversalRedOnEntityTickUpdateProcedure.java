...
public class ReversalRedOnEntityTickUpdateProcedure {
			...
			if (isMaxCharge) {
				...
				if (redLife % 2 == 0 && world instanceof Level level && !level.isClientSide()) {
					...
					for (Entity target : nearbyTargets) {
						...
						if (caster != null) {
							auraSource = new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("jjk_strongest:technique_red"))), caster);
						} else {
				...
				if (redLife >= 2 && redLife % 2 == 0) {
					if (world instanceof Level level && !level.isClientSide()) {
						level.playSound(null, BlockPos.containing(x, y, z), net.minecraft.core.registries.BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("entity.generic.explode")), SoundSource.NEUTRAL, 0.5F, 1.2F);
					}
	...
	private static void triggerExplosion(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (world instanceof Level level && !level.isClientSide()) {
			...
			level.playSound(null, BlockPos.containing(x, y, z), net.minecraftforge.registries.ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.generic.explode")), SoundSource.HOSTILE, volume, pitch);
		}
		// find caster
		Entity caster = null;
