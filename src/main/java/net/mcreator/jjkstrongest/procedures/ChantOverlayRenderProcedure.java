...
public class ChantOverlayRenderProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			ChantOverlayRenderProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				ChantOverlayRenderProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		...
		if (Minecraft.getInstance().player != null) {
			...
			if (R > 0) {
				...
				if (target(2)) {
					renderShape(ChargeRingShapeProcedure.execute(), (Minecraft.getInstance().getWindow().getGuiScaledWidth() / 2 - 0), (Minecraft.getInstance().getWindow().getGuiScaledHeight() / 2 - 0), 0, 0, 0, 0,
