...
public class FugaDomainExplosionExecuteProcedure {
	...
	private static void executeFugaExplosion(ServerLevel world, MalevolentShrineEntity shrine, Entity caster) {
		...
		DamageSource jujutsuDamage = new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("jjk_strongest:technique_fuga"))), caster);
