...
public class MahoragaBarrageProcedure {
	// close-range multi hit; combos into SLAM if target stays close
	public static void execute(LevelAccessor world, Entity entity, LivingEntity target) {
		...
		if (t >= 55) {
			...
			// combo: if target is still close and slam is off cooldown, chain directly
			boolean canChainSlam = entity.getPersistentData().getInt("maho_cd_slam") <= 0
					&& entity.distanceTo(target) <= 4.5;
			if (canChainSlam && entity.level().getRandom().nextDouble() < 0.55) {
