...
import net.efkrdnz.jjkstrongest.client.renderer.PurpleChargeLightningRenderer;
...
public class PurpleChargeLightningRenderer {
	...
	private static final ResourceLocation LIGHTNING_TEXTURE = ResourceLocation.parse("jjk_strongest:textures/entities/lightning_bolt.png");
	...
	public static void onRenderWorld(RenderLevelStageEvent event) {
		...
		for (Player player : players) {
			...
			if (chargeTicks > 0) {
				renderPurpleLightning(player, poseStack, bufferSource, event.getPartialTick().getGameTimeDeltaPartialTick(false), cameraPos, chargeTicks);
