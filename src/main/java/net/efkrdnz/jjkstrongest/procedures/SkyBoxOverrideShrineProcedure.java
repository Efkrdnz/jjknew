...
public class SkyBoxOverrideShrineProcedure {
	...
	public static void renderSky(boolean deepSky, boolean sunlights, boolean sun, boolean moon, boolean stars, boolean abyss) {
		...
		if (sun) {
			RenderSystem.setShaderTexture(0, ResourceLocation.parse("minecraft:textures/environment/sun.png"));
			RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
			renderSun(60.0F, 255 << 24 | 255 << 16 | 255 << 8 | 255, false);
		}
		if (moon) {
			RenderSystem.setShaderTexture(0, ResourceLocation.parse("minecraft:textures/environment/moon_phases.png"));
			RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != SkyBoxOverrideShrineProcedure.amount || seed != SkyBoxOverrideShrineProcedure.seed) {
			SkyBoxOverrideShrineProcedure.amount = amount;
