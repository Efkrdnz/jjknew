...
public class RaycastDismantleAdvancedProcedure {
	...
	private static void applySlashDamage(Level world, Entity shooter, Entity target, double damage, boolean isVertical, double techniquePower, boolean canFreeze) {
		...
		if (!world.isClientSide()) {
			world.playSound(null, BlockPos.containing(shooter.getX(), shooter.getY(), shooter.getZ()), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("jjk_strongest:kai")), SoundSource.NEUTRAL, 1, (float) 0.23);
		} else {
			world.playLocalSound((shooter.getX()), (shooter.getY()), (shooter.getZ()), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("jjk_strongest:kai")), SoundSource.NEUTRAL, 1, (float) 0.25, false);
		}
		...
		// apply damage
		DamageSource damageSource = new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("jjk_strongest:technique_dismantle"))), shooter);
