	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		if (world instanceof Level _level) {
			if (!_level.isClientSide()) {
				_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("jjk_strongest:blackflash")), SoundSource.NEUTRAL, (float) 0.3,
						(float) Mth.nextDouble(RandomSource.create(), 0.75, 1.25));
			} else {
				_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("jjk_strongest:blackflash")), SoundSource.NEUTRAL, (float) 0.3, (float) Mth.nextDouble(RandomSource.create(), 0.75, 1.25), false);
			}
		}
		// trigger shader effect
		if (world.isClientSide()) {
			TriggerChargedImpactProcedure.execute((Level)world, sourceentity, 3, 1.0f, 1.77f, 2.65f);
		}
	}
}
