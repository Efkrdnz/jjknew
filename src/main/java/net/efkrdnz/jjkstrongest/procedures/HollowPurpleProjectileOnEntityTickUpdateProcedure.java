	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity.getPersistentData().getString("state")).equals("move")) {
			...
			if (entity.getPersistentData().getDouble("PurpleLife") >= 4) {
				if (entity.getPersistentData().getDouble("PurpleLife") % 2 == 0) {
				...
				{
					final Vec3 _center = new Vec3(x, y, z);
					List<Entity> _entfound = world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate((6 * entity.getPersistentData().getDouble("TechniquePower") * 2) / 2d), e -> true).stream()
					...
					for (Entity entityiterator : _entfound) {
						if (!(entity.getPersistentData().getString("caster")).equals(entityiterator.getDisplayName().getString()) && !(entity == entityiterator)) {
							entityiterator.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("jjk_strongest:jujutsu")))), 75);
						}
					}
				}
			}
