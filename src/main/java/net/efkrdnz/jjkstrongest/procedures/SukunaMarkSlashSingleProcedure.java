...
public class SukunaMarkSlashSingleProcedure {
	public static void execute(LevelAccessor world, Player player, Entity target) {
		...
		DamageSource damageSource = new DamageSource(_world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.parse("jjk_strongest:technique_cleave"))), player);
