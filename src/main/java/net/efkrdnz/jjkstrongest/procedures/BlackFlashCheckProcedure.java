...
public class BlackFlashCheckProcedure {
	...
	private static void execute(@Nullable Event event, Level world, double x, double y, double z, Entity entity, Entity sourceentity, DamageSource damageSource, Entity immediatesourceentity) {
		...
		if (sourceentity.getType().is(TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.parse("technique")))) {
			return;
		}
		if (!(immediatesourceentity == null) && immediatesourceentity.getType().is(TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.parse("technique")))) {
			return;
		}
